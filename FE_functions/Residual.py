...
def assemble_residual(u, sigma_q, traction, V, num_cells, num_qp,
                      basis_grad, qp_weights,
                      right_face_dofs,
                      extra_tractions=None):
    """
    extra_tractions: list of (face_dofs, traction_vector) tuples
                     for applying traction on additional faces
                     e.g. [(top_face_dofs, [0, P, 0])] for biaxial
    """
    ...
    # ── Primary traction (right face, x-direction) ────────────────────────────
    num_right = len(right_face_dofs)
    for dof in right_face_dofs:
        R[dof * 3 + 0] -= traction[0] / num_right
        R[dof * 3 + 1] -= traction[1] / num_right
        R[dof * 3 + 2] -= traction[2] / num_right

    # ── Extra tractions (additional faces for biaxial, shear etc.) ────────────
    if extra_tractions is not None:
        for face_dofs, face_traction in extra_tractions:
            num_face = len(face_dofs)
            for dof in face_dofs:
                R[dof * 3 + 0] -= face_traction[0] / num_face
                R[dof * 3 + 1] -= face_traction[1] / num_face
                R[dof * 3 + 2] -= face_traction[2] / num_face
