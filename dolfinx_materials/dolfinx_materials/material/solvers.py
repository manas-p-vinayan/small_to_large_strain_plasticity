...
class NonlinearMaterialProblem(NonlinearProblem):
    ...
    def __init__(self, qmap, F, J, u, bcs, petsc_options_prefix="nls_"): ## change
        """
        Parameters
        ----------
        ...
        bcs : list
            list of fem.dirichletbc
        """
        super().__init__(F, u, J=J, bcs=bcs,petsc_options_prefix=petsc_options_prefix) ## change
        #self._F = None
        #self._J = None
        self.u_ref = u ##change
        self.bcs = bcs               # keep BCs around
        if not isinstance(qmap, list):
            self.quadrature_maps = [qmap]
        else:
            self.quadrature_maps = qmap
        # --- Compatibility alias for newer NewtonSolver that asks for ".a"
    @property
    def a(self):
        return self.J
    ...
    def matrix(self):
        return create_matrix(self.J) # allocate from the real Jacobian form before it was .a
        # Allocate from the actual Jacobian (C++ Form)
        #return cpp.fem.petsc.create_matrix(self.J._cpp_object)

    def vector(self):
        # Allocate from the actual residual (C++ Form)
        return create_vector(self.F) ## allocate from the real residual form before it was .L
        #return cpp.fem.petsc.create_vector(self.F._cpp_object)
    ...
    def solve(self, solver, print_solution=True):
       ...
       # solver.setF(self.F._cpp_object, self.vector())
       # solver.setJ(self.J._cpp_object, self.matrix())
...
class NonlinearMaterialProblemNew(NonlinearProblem):
    """
    Nonlinear problem with a QuadratureMap constitutive update hook,
    compatible with dolfinx NewtonSolver, like the Bleyer demo.
    """
    ...
    def __init__(
        ...
        # Build dolfinx NonlinearProblem (it will compile forms internally)
        super().__init__(F, u, J=J, bcs=bcs, petsc_options_prefix=petsc_options_prefix,
                         petsc_options=petsc_options)

        self.u_ref = u
        self.bcs = bcs
        self.quadrature_maps = [qmap] if not isinstance(qmap, list) else qmap

        # Create and store the Newton solver (so you can call problem.solve())
        self.solver = NewtonSolver(u.function_space.mesh.comm, self)

        # If you pass PETSc options, apply them through the prefix
        # (NewtonSolver reads PETSc options via prefix; this is how the demo does it)
        self.solver.setOptionsPrefix(petsc_options_prefix)
        self.solver.setFromOptions()

    # Newer APIs sometimes expect `.a` as Jacobian form
    @property
    def a(self):
        return self.J
    ...
    def form(self, x):
        # Called by NewtonSolver before residual/J assembly
        x.ghostUpdate(addv=PETSc.InsertMode.INSERT, mode=PETSc.ScatterMode.FORWARD)
        self._constitutive_update()

    def matrix(self):
        return create_matrix(self.J)

    def vector(self):
        return create_vector(self.F)

    def solve(self, print_solution=True):
        solver = self.solver
        ...
        if converged:
            ...
            mpiprint(f"No solution found after {it} iterations.")
